...
```
scripts/
├── lighthouse-audit.js      # Lighthouse CLI 자동화
└── performance-profile.js   # React DevTools Profiler 데이터 수집

docs/performance/
└── frontend-baseline-2025-11-04.md  # 기준치 보고서
```

**측정 지표**:
| 지표 | 목표 | 현재 | 상태 |
|------|------|------|------|
| Lighthouse 성능 점수 | ≥90 | ? | ⏳ 측정 예정 |
| First Contentful Paint | <1.5s | ? | ⏳ 측정 예정 |
| Time to Interactive | <3.0s | ? | ⏳ 측정 예정 |
| Total Blocking Time | <200ms | ? | ⏳ 측정 예정 |
| Cumulative Layout Shift | <0.1 | ? | ⏳ 측정 예정 |
| Bundle Size (gzip) | <500KB | ? | ⏳ 측정 예정 |

**예상 소요 시간**: 1일

**성공 기준**:
- ✅ Lighthouse 자동화 스크립트 작성
- ✅ 모든 Core Web Vitals 측정
- ✅ 최적화 기회 5개 이상 발견

---

#### Task 1.4: 기준치 보고서 작성 ⏳ **대기 중**
...
**생성할 파일**:
```
docs/performance/baseline-report-2025-11-04.md
```
